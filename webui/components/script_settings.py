...
# ============================================================================
# 🛡 FILE UPLOAD VALIDATION — Security + Performance
# ============================================================================
VIDEO_MAX_SIZE_MB = 2000        # Main Video Upload (2 GB — Streamlit limit)
AUDIO_MAX_SIZE_MB = 50          # Audio Reference Upload
SUBTITLE_MAX_SIZE_MB = 10       # SRT/Subtitle Upload
IMAGE_MAX_SIZE_MB = 20          # Logo/Image Upload

VIDEO_EXTENSIONS = {".mp4", ".mov", ".avi", ".flv", ".mkv", ".mpeg4", ".webm", ".m4v"}
...
def validate_upload(
    ...
    """File Upload ကို စစ် — Extension + Size + Security

    Args:
        uploaded_file: Streamlit UploadedFile
        allowed_extensions: ခွင့်ပြုတဲ့ Extension Set (e.g. {'.mp4', '.mov'})
        max_size_mb: အများဆုံး Size (MB)
        file_label: User ကို ပြရန် နာမည် (e.g. "Video")

    Returns:
        (True, "") — အိုကေ
        (False, "error message") — Fail
    """
    if uploaded_file is None:
        return False, f"⚠️ {file_label} — File မရွေးရသေး"

    # --- Filename Check ---
    filename = str(uploaded_file.name or "").strip()
    if not filename:
        return False, f"⚠️ {file_label} — Filename မမှန်ကန်ဘူး"

    # --- Path Traversal Check ---
    if ".." in filename or "/" in filename or "\\" in filename:
        return False, f"❌ {file_label} — Filename မလုံခြုံဘူး"
    ...
    if ext not in allowed_extensions:
        ...
        return False, (
            f"❌ {file_label} — Extension `{ext}` မခွင့်ပြုဘူး\n\n"
            f"✅ ခွင့်ပြုတာ: {allowed_str}"
        )
    ...
    if size_bytes > 0:
        ...
        if size_mb > max_size_mb:
            return False, (
                f"❌ {file_label} — Size အလွန်ကြီးတယ်\n\n"
                f"📊 Upload: **{size_mb:.1f} MB**\n"
...
def render_video_file(tr, params):
    ...
    if source_options[source] == "upload":
        ...
        if not uploaded_files:
            ...
            # ================================================================
            # 🛡 VALIDATION — File Upload စစ်ဆေး
            # ================================================================
            _validation_failed = False
            for _uf in uploaded_files:
                _ok, _err = validate_upload(
                    _uf,
